    ...
    <SectionLayout section="lobby">
      <AnimatePresence>
        {currentSection === "lobby" && (
          <motion.div
            className="w-full h-full pt-8 flex flex-col relative lg:w-[70vw]"
            initial="hidden"
            animate="visible"
            exit="exit"
            variants={{
      ...
      {isRestricted && (
        <div className="absolute start-0 right-0 top-0 bottom-0 bg-black/80 z-10 h-full max-h-screen w-screen flex flex-col justify-center items-center gap-4">
          <div className="flex flex-col justify-center items-center gap-4 mb-14">
            <div className="text-[12px] text-center paragraph w-[70%]">
              The lobby is restricted to patrons who have paid their deposit.
              Please pay your deposit in order to enter the Lobby
            </div>
